...
- 报告建议只能接收当前 `confirmed: true` 的指标；未确认字段不得进入 AI 输入、fallback、引用 ID 或展示证据。修改指标后 App 必须明确标记旧建议需要更新，并在用户确认保存后调用 regenerate;
...
- 错误码:`UNAUTHORIZED`、`VALIDATION_ERROR`、`INVALID_IMAGE`、`AI_UNAVAILABLE`、`AI_TIMEOUT`、`MALFORMED_AI_OUTPUT`、`REPORT_CHANGED`、`NOT_FOUND`、`RATE_LIMITED`、`INTERNAL`。`REPORT_CHANGED` 表示建议生成期间报告指标已被再次修改，客户端应重新加载或重试生成。
