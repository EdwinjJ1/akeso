...
export function CheckInPrompt({ mode = 'first' }: CheckInPromptProps) {
  const isDailyUpdate = mode === 'daily'
  const kicker = isDailyUpdate ? 'A QUICK DAILY REFRESH' : "20 SECONDS, THAT'S IT"
  const title = isDailyUpdate ? "Update today's status" : "How's your energy, really?"
  const subtitle = isDailyUpdate
    ? 'Your last answers are ready. Keep what still feels true and change what is different today.'
    : "A 20-second check-in unlocks your energy score, today's plan, and meals matched to what your body needs."
  const buttonLabel = isDailyUpdate ? "Update today's status" : 'Start check-in'
